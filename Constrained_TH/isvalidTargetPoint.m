...
if ((TP(1)-origin(1))^2+(TP(2)-origin(2))^2) <= (parameters.a1+parameters.a2+parameters.a3)^2 || ...
        (TP(1)-parameters.circle.x)^2 + (TP(2)-parameters.circle.y)^2 > parameters.circle.r^2
    fprintf('Valid Target Point!\n')
    output = true;
else
    fprintf('Invalid Target Point for the current manipulator configuration\nInsert a Target Point in the operating condition or outside the circle and try again.\n')
    output = false;
end
